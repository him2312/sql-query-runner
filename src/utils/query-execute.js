...
export const SQL_QUERY_VALIDATOR =
...
export const queryExecutor = (query) => {
  if (SQL_QUERY_VALIDATOR.test(query)) {
    ...
    let dirtyFields = query.match(CAPTURE_FIELD_NAME);
    if (tableNameMatches) {
      return {
        tableName: tableNameMatches[0].trim(),
        fields: parseFields(dirtyFields),
      };
    }
  } else {
...
const parseFields = (fields) => {
  let trimFields = fields[0].trim();
  let allFields = trimFields.split(",").map((field) => field.trim());
  return allFields || [];
};
...
export const getFilteredDataFromTable = (tableName, fields = []) => {
  ...
  let filteredHeader = removeKeysFromObject(filteredFields, DATABASE_MAPPING[tableName].header);
